const DATA_URL='/articles/data/articles.json';const MORE_URL='/articles/data/articles-more.json';
...
Promise.all([fetch(DATA_URL).then(r=>r.json()),fetch(MORE_URL).then(r=>r.json())]).then(([base,more])=>{allArticles=[...(base.articles||[]),...(more.articles||[])];render()}).catch(()=>{$('#empty').hidden=false});
