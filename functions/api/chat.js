...
export async function onRequestGet({ request, env }) {
  return json({ service: 'Redlighte AI', status: env.OPENROUTER_API_KEY ? 'ready' : 'not_configured' }, 200, corsHeaders(request));
}

export async function onRequestPost({ request, env }) {
  const origin = request.headers.get('Origin');
  if (origin && !isAllowedOrigin(origin)) return json({ error: 'Origin not allowed.' }, 403, corsHeaders(request));
  if (!env.OPENROUTER_API_KEY) return json({ error: 'AI service is not configured.' }, 503, corsHeaders(request));

  let body;
  try { body = await request.json(); } catch { return json({ error: 'Invalid JSON body.' }, 400, corsHeaders(request)); }
  ...
  if (input.length > MAX_MESSAGE_LENGTH) return json({ error: 'Message is too long.' }, 413, corsHeaders(request));
  ...
  try {
    ...
    return json({ error: 'Could not reach the AI provider.' }, 502, corsHeaders(request));
  }
  ...
  if (typeof answer !== 'string' || !answer.trim()) return json({ error: 'The AI service returned an empty response.' }, 502, corsHeaders(request));

  return json({ message: answer, model: result.model || MODEL }, 200, corsHeaders(request));
}
