...
function load(){try{const x=JSON.parse(localStorage.getItem(key())||'{}');data={...data,...x,achievements:Array.isArray(x.achievements)?x.achievements:[],playedOriginals:Array.isArray(x.playedOriginals)?x.playedOriginals:[]}}catch{}}
...
async function connectAccount(){
 try{const r=await fetch('/api/auth/me',{credentials:'include',headers:{Accept:'application/json'}});if(!r.ok)throw new Error('unauthenticated');const j=await r.json();player=j.user||j.account||j;if(!player||(!player.username&&!player.name&&!player.id))throw new Error('empty');load();$('#accountName').textContent=player.name||player.username||'Redlighte Account';$('#avatar').textContent=(player.name||player.username||'?').trim().charAt(0).toUpperCase();$('#accountPill').title=`Connected as ${player.name||player.username}`;render();}
 catch{player=null;$('#accountName').textContent='Sign in to Redlighte';$('#avatar').textContent='→';$('#accountPill').href='/?account=login';notify('Sign in to connect your Redlighte Account');render()}
}
