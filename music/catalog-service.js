...
import { searchStored } from './storage.js';

export async function searchCatalog(env, term) {
  const q = String(term || '').trim();
  if (!q) return { songs: [], artists: [], albums: [], sources: [] };
  const stored = await searchStored(env, q, 25);
  if (stored?.songs?.length) return stored;
  ...
  return {
    songs: dedupe([...mb.songs, ...adb.songs]),
    artists: dedupe([...mb.artists, ...adb.artists]),
    albums: dedupe([...mb.albums, ...adb.albums]),
    sources: [mbResult.status === 'fulfilled' ? 'musicbrainz' : null, adbResult.status === 'fulfilled' ? 'theaudiodb' : null].filter(Boolean)
  };
}
