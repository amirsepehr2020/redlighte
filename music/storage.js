export function hasDb(env) { return Boolean(env?.MUSIC_DB && typeof env.MUSIC_DB.prepare === 'function'); }
...
export async function upsertCatalog(env, data) {
  ...
  const db=env.MUSIC_DB, artists=data.artists||[], albums=data.albums||[], songs=data.songs||[], statements=[];
  ...
  for(const s of songs){const source=s.provider?{...s}:null;if(source) statements.push(db.prepare('INSERT INTO sources (id,song_id,provider,page_url,audio_url,cover_url,status,last_checked) VALUES (?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET page_url=excluded.page_url,audio_url=excluded.audio_url,cover_url=excluded.cover_url,status=excluded.status,last_checked=CURRENT_TIMESTAMP').bind(`${s.id}:${source.provider}`,s.id,source.provider,source.page_url||null,source.audio_url||null,source.cover_url||s.cover_url||null,source.status||'active'));}
...
export async function getStored(env,type,id){
  if(!hasDb(env)) return null;
  const table=type==='song'?'songs':type==='artist'?'artists':type==='album'?'albums':null; if(!table)return null;
  return await env.MUSIC_DB.prepare(`SELECT * FROM ${table} WHERE id=? OR slug=? LIMIT 1`).bind(id,id).first();
}
