(()=>{
  ...
  const ready=()=>{
    ...
    const syncChatHistory=()=>{
      if(!chat.hidden&&!chatHistoryPushed){history.pushState({redlighteChat:true},'',location.href);chatHistoryPushed=true;}
    ...
    const openDeepLink=()=>{
      ...
      const id=params.get('chat')||hash;if(!id)return;
      let historyItems=[];try{historyItems=JSON.parse(localStorage.getItem('redlighte_history')||'[]')}catch{}
      const item=historyItems.find(x=>x.id===id);if(!item)return;
      const tryOpen=()=>{
    ...
    const setNetwork=online=>{
      if(!networkBanner){networkBanner=document.createElement('div');networkBanner.className='redlighte-network-banner';networkBanner.setAttribute('role','status');document.body.append(networkBanner)}
      networkBanner.textContent=online?'Connection restored':'You are offline';
      networkBanner.classList.toggle('offline',!online);networkBanner.classList.add('visible');
      clearTimeout(setNetwork.timer);setNetwork.timer=setTimeout(()=>networkBanner.classList.remove('visible'),online?1800:0);
    };
    addEventListener('offline',()=>setNetwork(false));addEventListener('online',()=>setNetwork(true));if(!navigator.onLine)setNetwork(false);
