...
const memoryContext=buildMemoryContext(selectedMemories);const systemMessage=memoryContext?`${SYSTEM_PROMPT}\n\n${memoryContext}`:SYSTEM_PROMPT;try{const result=await env.AI.run(MODEL,{messages:[{role:'system',content:systemMessage},...messages],max_tokens:4096,temperature:.45});const answer=result?.response||result?.result?.response;if(typeof answer!=='string'||!answer.trim())return json({error:'The AI service returned an empty response.',code:'EMPTY_AI_RESPONSE',request_id:requestId},502,cors);let memoryEvent=null;if(session&&memoryData?.enabled){try{memoryEvent=await extractAndMergeMemory(env,session.username,input,messages,body?.conversation_id||null)}catch(error){console.error(`[${requestId}] MEMORY_SAVE_ERROR`,error)}}const finalMessage=memoryEvent?.memory?.content?`${answer.trim()}\n\n🧠 ${memoryEvent.action==='updated'?'Memory updated':'Saved to Memory'} — ${memoryEvent.memory.content}`:answer;return json({message:finalMessage,model:MODEL,request_id:requestId,memory:memoryEvent?{action:memoryEvent.action,type:memoryEvent.memory.type,id:memoryEvent.memory.id}:null},200,cors)}catch(error){console.error(`[${requestId}] WORKERS_AI_ERROR`,error);return json({error:'AI service error.',code:'WORKERS_AI_ERROR',detail:error?.message||String(error),request_id:requestId},502,cors)}}
