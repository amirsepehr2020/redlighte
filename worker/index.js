...
async function logDeviceInput(request,env,user){try{const path=`device-input/${user.username}.json`;const file=await githubFile(env,path);const data=file?JSON.parse(file.content):{account:{id:user.id,name:user.name,username:user.username},logs:[]};if(!data.account)data.account={id:user.id,name:user.name,username:user.username};if(!Array.isArray(data.logs))data.logs=[];const cf=request.cf||{};const ua=request.headers.get('User-Agent')||'';const clientModel=request.headers.get('Sec-CH-UA-Model')||'';const platform=request.headers.get('Sec-CH-UA-Platform')||'';const mobile=request.headers.get('Sec-CH-UA-Mobile')||'';const language=request.headers.get('Accept-Language')||'';const model=clientModel||extractDeviceModel(ua)||'';const deviceType=mobile==='?1'||/Mobile|Android.*Mobile|iPhone|iPod/i.test(ua)?'mobile':/iPad|Tablet|Android(?!.*Mobile)/i.test(ua)?'tablet':'desktop';const deviceName=model||(/iPhone/i.test(ua)?'iPhone':/iPad/i.test(ua)?'iPad':deviceType);data.logs.push({timestamp:new Date().toISOString(),ip:request.headers.get('CF-Connecting-IP')||'',timezone:cf.timezone||'',device:{type:deviceType,name:deviceName,model:model||deviceName,modelSource:clientModel?'client-hint':extractDeviceModel(ua)?'user-agent':'browser-unavailable',os:platform||'',browser:parseBrowser(ua),userAgent:ua,language,clientHints:{platform,mobile,model:clientModel}}});await githubWrite(env,path,data,file?.sha||null,'Log device input')}catch(error){console.error('DEVICE_INPUT_LOG_ERROR',error)}}
function extractDeviceModel(ua){const android=ua.match(/Android[^;)]*;\s*([^;)]+?)(?:\s+Build\/[^;)]+)?[;)]/i);if(android&&android[1]&&!/wv|mobile|tablet/i.test(android[1]))return android[1].trim();if(/iPhone/i.test(ua))return'iPhone';if(/iPad/i.test(ua))return'iPad';if(/iPod/i.test(ua))return'iPod';return''}
