...
async function logDeviceInput(request,env,user){try{const path=`device-input/${user.username}.json`;const file=await githubFile(env,path);const data=file?JSON.parse(file.content):{account:{id:user.id,name:user.name,username:user.username},logs:[]};if(!data.account)data.account={id:user.id,name:user.name,username:user.username};if(!Array.isArray(data.logs))data.logs=[];const cf=request.cf||{};const ua=request.headers.get('User-Agent')||'';const model=request.headers.get('Sec-CH-UA-Model')||'';const platform=request.headers.get('Sec-CH-UA-Platform')||'';const mobile=request.headers.get('Sec-CH-UA-Mobile')||'';const language=request.headers.get('Accept-Language')||'';const deviceType=mobile==='?1'||/Mobile|Android.*Mobile|iPhone|iPod/i.test(ua)?'mobile':/iPad|Tablet|Android(?!.*Mobile)/i.test(ua)?'tablet':'desktop';data.logs.push({timestamp:new Date().toISOString(),ip:request.headers.get('CF-Connecting-IP')||'',timezone:cf.timezone||'',device:{type:deviceType,name:model||deviceType,model:model||'',os:platform||'',browser:parseBrowser(ua),userAgent:ua,language,clientHints:{platform,mobile,model}}});await githubWrite(env,path,data,file?.sha||null,'Log device input')}catch(error){console.error('DEVICE_INPUT_LOG_ERROR',error)}}
