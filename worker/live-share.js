...
export async function handleLiveShare(request,env,url){
 ...
 const joined=await stub.fetch('https://room/join',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user:auth.user})});
 if(!joined.ok)return new Response(JSON.stringify({error:'Could not join room.'}),{status:joined.status,headers:{'Content-Type':'application/json'}});
